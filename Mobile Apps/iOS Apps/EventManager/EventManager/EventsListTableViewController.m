...
- (void)loadInitialData {
        ...
        [self.manager createEvent:firstEvent];
        [self.manager createEvent:secondEvent];
...
- (IBAction)unwindToList:(UIStoryboardSegue *)segue
{
    ...
    if (event != nil) {
        [self.manager createEvent:event];
...
- (void)viewDidLoad {
    ...
    self.manager = [[EventManager alloc] init];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return [self.manager.events count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Event *event = [self.manager.events objectAtIndex:indexPath.row];
